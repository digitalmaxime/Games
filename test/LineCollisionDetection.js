import calculateForceWall from '/forceCalculation.js';

function addForceToBall (ball, angle, linePosition) {
    const velocity = calculateForceWall(ball, angle, linePosition);
...
export default function detectLineCollision(ball, angle, xorigin, yorigin, linePosition) {    
    ...
    if (angle < 0) {
        switch(linePosition) {
            case 0: //topLeft
                if (cos*ball.radius + yorigin - ball.x  > ball.y - cos*ball.radius - xorigin) {
